...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    {
        Vertex vertex;
        // Обрабатываем координаты вершин
        glm::vec3 vector;
        vector.x = mesh->mVertices[i].x;
        vector.y = mesh->mVertices[i].y;
        vector.z = mesh->mVertices[i].z;
        vertex.Position = vector;
        // Обрабатываем координаты нормали
    ...
    int aaaaaaaaaaaaa = mesh->mMaterialIndex;
...
unsigned int Model::TextureFromFile(char const* path, const std::string& directory)
{
